...
OUTPUT_DIR = Path(
    "/afs/cern.ch/work/y/ykim/project_birkslaw/analysis/results_selected"
)
...
def analyze_dataset(files, tree_name, max_events):
    """Extract QDC values from selected beam-candidate events."""
    ...
    # Protect against accidentally scanning an extremely large data sample
    # while testing the selection.
    maximum_events_to_scan = min(available_events, 100000)

    print(f"  Input files: {len(files)}")
    print(f"  Available events: {available_events}")
    print(f"  Maximum events to scan: {maximum_events_to_scan}")
    ...
    for event_index in range(maximum_events_to_scan):
        if len(total_us_qdc) >= max_events:
            break
        ...
        # Select events with a positive signal in bar 4 or 5
        # in each of US1, US2 and US3.
        if planes_with_target_hit != required_planes:
            continue

        total_us_qdc.append(event_total)

        # Store channel values only after the event passes selection.
        for key, values in event_channel_qdc.items():
            channel_qdc[key].extend(values)

    print(f"  Scanned events: {scanned_events}")
    print(f"  Selected events: {len(total_us_qdc)}")

    if len(total_us_qdc) < max_events:
        print(
            "  WARNING: Fewer selected events than requested."
        )

    return {
        "events": len(total_us_qdc),
